<template>
    ...
    <a-table :columns="columns" :data-source="papers">
        ...
        <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'action'">
                <span>
                    <!--下载请求-->
                    <a :href="'http://localhost:9875/api/download?uuid=' + record.paper" target="_blank">下载文章</a>
    ...
    <a-drawer width="800" placement="right" :closable="false" :visible="visible" @close="onClose">
        ...
        <a-row>
            <a-col :span="3">
                <a-button type="primary" :href="'http://localhost:9875/api/download?uuid=' + paper.uuid"
                    target="_blank">
                    <template #icon>
                        <DownloadOutlined />
                    </template>
